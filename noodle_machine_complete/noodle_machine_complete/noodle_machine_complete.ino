...
/************************* Main Mission Start ********************************/
  if (digitalRead(Button_START_PIN) == 1) {
    ...
    if (mode == 1) {                                      
      ...
      /* 1. SM#1 3.5 cycles */
      Serial.println("Mode A --- Step 1 : SM#1 3.5 cycles");
      ...
      digitalWrite(EV_1_PIN,LOW);
      /* 4. Turn EV#2 on for one sec */
      Serial.println("Mode A --- Step 4 : Turn EV#2 on for one sec");
      digitalWrite(EV_2_PIN,HIGH);
      delay(1000);                                        // 1 sec
      digitalWrite(EV_2_PIN,LOW);
      /* 5. Delay 1 sec */
      Serial.println("Mode A --- Step 5 : Delay 1 sec");
      delay(1000);
      /* 6. SM#2 2.75 cycles */
      Serial.println("Mode A --- Step 6 : SM#2 2.75 cycles");
      for (int i = 0; i < 2.75*cycle; i++) {                 // 2.75 cycle
      ...
      /* 8. SM#2 2.75 cycle */
      Serial.println("Mode A --- Step 8 : SM#2 2.75 cycles");
      for (int i = 0; i < 2.75*cycle; i++) {               // 2.75 cycle
        digitalWrite(SM_2_STEP_PIN,HIGH);
        delayMicroseconds(speed_2);
        digitalWrite(SM_2_STEP_PIN,LOW);
      ...
      /* 10. SM#2 3 cycles */
      Serial.println("Mode A --- Step 10 : SM#2 2 cycles");
      for (int i = 0; i < 3*cycle; i++) {               // 3 cycle
        digitalWrite(SM_2_STEP_PIN,HIGH);
        delayMicroseconds(speed_2);
        digitalWrite(SM_2_STEP_PIN,LOW);
        delayMicroseconds(speed_2);
      ...
      delay(10000);                                       // 10 sec
      digitalWrite(EV_3_PIN,LOW);
      /* 12. SM#2 8.5 inverse cycles */
      Serial.println("Mode A --- Step 12 : SM#2 8.5 inverse cycles");
      digitalWrite(SM_2_DIR_PIN,HIGH);                    // inverse SM#2 direction
      for (int i = 0; i < 8.5*cycle; i++) {                // 8.5 cycle
        digitalWrite(SM_2_STEP_PIN,HIGH);
        delayMicroseconds(speed_2);
        digitalWrite(SM_2_STEP_PIN,LOW);
        delayMicroseconds(speed_2);
      ...
      digitalWrite(SM_2_DIR_PIN,LOW);
      mode = 0;
      Serial.println("Mode A finished.");
    }
    /************************* MODE B *****************************/
    else if ( mode == 2) { 
      Serial.println("Starting mode B......");
      /* 1. SM#1 3.5 cycles */
      Serial.println("Mode B --- Step 1 : SM#1 3.5 cycles");
      digitalWrite(SM_1_DIR_PIN,HIGH);
      for (int i = 0; i <3.5*cycle; i++) {                 // 3.5 cycle
        digitalWrite(SM_1_STEP_PIN,HIGH);
        delayMicroseconds(speed_1);
      ...
      digitalWrite(EV_1_PIN,LOW);
      /* 4. Turn EV#2 on for one sec */
      Serial.println("Mode B --- Step 4 : Turn EV#2 on for one sec");
      digitalWrite(EV_2_PIN,HIGH);
      delay(1000);                                        // 1 sec
      digitalWrite(EV_2_PIN,LOW);
      /* 5. Delay 1 sec */
      Serial.println("Mode B --- Step 5 : Delay 1 sec");
      delay(1000);
      /* 6. SM#2 5.5 cycles */
      Serial.println("Mode B --- Step 6 : SM#2 5.5 cycles");
      for (int i = 0; i < 5.5*cycle; i++) {               // 5.5 cycle
        digitalWrite(SM_2_STEP_PIN,HIGH);
        delayMicroseconds(speed_2);
        digitalWrite(SM_2_STEP_PIN,LOW);
        delayMicroseconds(speed_2);
      ...
      /* 8. SM#2 2.75 inverse cycles */
      Serial.println("Mode B --- Step 8 : SM#2 2.75 inverse cycles");
      digitalWrite(SM_2_DIR_PIN,HIGH);                    // inverse SM#2 direction
      for (int i = 0; i < 2.75*cycle; i++) {               // 2.75 cycle
        digitalWrite(SM_2_STEP_PIN,HIGH);
        delayMicroseconds(speed_2);
      ...
      /* 10. SM#2 5.75 cycles */
      Serial.println("Mode B --- Step 10 : SM#2 5.75 cycles");
      digitalWrite(SM_2_DIR_PIN,LOW);                      // reset SM#2 direction
      for (int i = 0; i < 5.75*cycle; i++) {                 // 5.75 cycle
        digitalWrite(SM_2_STEP_PIN,HIGH);
        delayMicroseconds(speed_2);
        digitalWrite(SM_2_STEP_PIN,LOW);
        delayMicroseconds(speed_2);
      ...
      /* 11. Turn EV#3 on for 10 sec */
      Serial.println("Mode B --- Step 11 : Turn EV#3 on for 10 sec");
      digitalWrite(EV_3_PIN,HIGH);
      delay(10000);                                      // 10 sec
      digitalWrite(EV_3_PIN,LOW);
      /* 12. SM#2 8.5 inverse cycles */
      Serial.println("Mode B --- Step 12 : SM#2 8.5 inverse cycles");
