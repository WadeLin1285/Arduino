...
#define SCHOOL_N 4        // number of school name
#define NUMBER_N 3        // number of machine number
...
byte server[] = { 34 , 235 , 10 , 162}; // maker.ifttt.com
byte server_backup[] = { 34 , 235 , 45 , 235}; // maker.ifttt.com
...
char code[CODE_N];
char school[SCHOOL_N+1];
char number[NUMBER_N+1];
char *sch = school,*num = number;
int data_count = 0;
...
void setup() {
  ...
  if (client.connect(server,80)) Serial.println(F("connected"));
    ...
    if (client.connect(server_backup,80)) Serial.println(F("connected"));
...
void loop() {
  ...
  if (digitalRead(triggerPin_1) == HIGH || digitalRead(triggerPin_2) == HIGH) {
    ...
    digitalWrite(redPin,LOW);     // red light off
    if (alert) digitalWrite(buzzerPin,HIGH);  //Setting buzzer pin to HIGH(buzzer off)
  }
